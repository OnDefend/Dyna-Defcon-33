...
class MLEnhancedSecretClassifier:
    ...
    def _generate_training_data(self) -> List[Dict[str, Any]]:
        ...
        true_secrets = [
            {'content': 'AKIA1234567890EXAMPLE', 'is_secret': True, 'context': 'aws configuration'},
            {'content': 'sample_test_key_placeholder', 'is_secret': True, 'context': 'stripe api key'},
            {'content': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9', 'is_secret': True, 'context': 'jwt token'},
            {'content': 'ghp_1234567890abcdefghijklmnopqrstuvwx', 'is_secret': True, 'context': 'github token'},
            {'content': 'xoxb-XXXXXXXXXX-XXXXXXXXXX-XXXXXXXXXXXXXXXX', 'is_secret': True, 'context': 'slack bot token'},
            {'content': 'AIzaSyABC123DEF456GHI789JKL012MNO345PQR', 'is_secret': True, 'context': 'google api key'},
            {'content': 'sample_live_key_placeholder', 'is_secret': True, 'context': 'live stripe key'},
        ]
    ...
    def _extract_ml_features(self, content: str, context: Optional[Dict[str, Any]] = None) -> np.ndarray:
        ...
        features.extend([
            ...
            1 if any(pattern in content for pattern in ['AKIA', 'sk_', 'ghp_', 'xoxb-']) else 0,
    ...
    def _rule_based_prediction(self, content: str, context: Optional[Dict[str, Any]] = None) -> Tuple[bool, float, Dict[str, Any]]:
        ...
        has_secret_patterns = any(pattern in content for pattern in ['AKIA', 'sk_', 'ghp_', 'xoxb-'])
