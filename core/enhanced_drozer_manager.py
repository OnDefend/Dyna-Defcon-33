#!/usr/bin/env python3
"""
Enhanced Drozer Connection Manager with Robust Error Handling

This module provides comprehensive drozer connection management with intelligent
error handling, connection pooling, automatic retry logic, and graceful degradation
to ensure security analysis continues even when drozer encounters connectivity issues.

Key Features:
- Intelligent connection state management
- ConnectionError attribute fix for Python 3.x compatibility
- Adaptive timeout based on application size and complexity
...
class EnhancedDrozerManager:
    ...
    def _execute_drozer_command_direct(self, command: str, timeout: Optional[int] = None) -> Tuple[bool, str]:
        ...
        """
        # Use shorter, more realistic timeout (reduced from adaptive_timeout)
        cmd_timeout = timeout or min(20, self.adaptive_timeout)  # Cap at 20 seconds
        full_command = f"drozer console connect --command '{command}'"
        
        try:
            self.logger.debug(f"🔨 Executing: {command} (timeout: {cmd_timeout}s)")
            
            # Use more robust process execution
            process = subprocess.Popen(
                ...
                text=True,
                start_new_session=True  # Prevent hanging
            )
            ...
            try:
                ...
                if process.returncode == 0:
                    ...
                    output = stdout.strip()
                    failure_indicators = [
                        "No devices connected", "not found", "Could not connect",
                        "Connection refused", "No route to host", "timeout", 
                        "error", "failed", "Unable to connect"
                    ]
                    
                    if any(indicator.lower() in output.lower() for indicator in failure_indicators):
                        return False, f"Drozer connection failed: {output}"
